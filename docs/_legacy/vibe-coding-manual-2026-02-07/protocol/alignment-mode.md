# Alignment Mode — Rawls Game

## Purpose
Alignment Mode is triggered when `docs/project/NEXT.md` is missing, unclear, or outdated. This prevents coding work when the active story/next step is ambiguous.

## When to Enter Alignment Mode

STOP coding and enter Alignment Mode if:
- `docs/project/NEXT.md` is missing
...
- [ ] Vision approved (`docs/project/VISION.md` exists or template completed; Non-Goals present)
- [ ] Active Epic + Active Story ID chosen
- [ ] `docs/project/NEXT.md` has Active Story + Next Step + DoD + Scope boundaries

**B) ChatGPT Approval (prompt planning):**
- [ ] ChatGPT confirms NEXT STEP is tiny/testable + matches current repo state
- [ ] Prompt will cite `docs/project/NEXT.md` (ACTIVE STORY + NEXT STEP)
...
If VISION/EPICS/NEXT exist elsewhere in the repo (scattered or misplaced), use the Doc Discovery + Migration section below to locate and migrate into docs/project/. Do not invent new content to pass gates; discover and migrate existing planning docs first.

## Exit Criteria

Alignment Mode completes when:
- `docs/project/VISION.md` exists (or placeholders created using templates)
- `docs/project/EPICS.md` exists (or placeholders created using templates)
- `docs/project/NEXT.md` exists with clear ACTIVE STORY ID, NEXT STEP, DoD, scope guardrails, "done when"
- Repo state verified (clean tree, tests pass, no conflicts with NEXT.md)

After Alignment Mode, resume normal coding workflow starting with the NEXT STEP from `docs/project/NEXT.md`.

## Doc Discovery + Migration (when Control Deck docs are missing)

### A) When to use (trigger)
- **Trigger:** Start-of-Session Doc Audit fails because `docs/project/VISION.md` and/or `EPICS.md` and/or `NEXT.md` are missing OR present-but-placeholders (TBD/TODO/empty).
...
- Previous handoffs in `docs/status/` and any `handoff-*.md`
...
### D) Migration rules (how to move content into docs/project/)
- **Create missing docs using templates:** Use `docs/project/*.template.md` (do NOT invent new formats).
- **If docs exist elsewhere:**
  - Copy the relevant content into the correct `docs/project/*` file(s).
...
- **VISION.md:** purpose + user promise + non-goals (no TBD).
- **EPICS.md:** at least 1 epic with an ID + short description + status.
- **NEXT.md:** Active Story ID + NEXT STEP + DoD + scope guardrails + Done When checklist (no TBD).
...
### When to use (trigger)
- **Trigger:** Start-of-Session Doc Audit passes existence check (VISION.md, EPICS.md, NEXT.md all exist) BUT Population Gate **FAIL** (placeholders detected or minimum content requirements not met).
...
Commands:
- `grep -i "TBD\|TODO\|TEMPLATE\|placeholder" docs/project/VISION.md docs/project/EPICS.md docs/project/NEXT.md`
- `rg -i "TBD|TODO|TEMPLATE|placeholder" docs/project/`

### Questions to ask Stephen (minimum to populate)
1. **VISION.md placeholders:**
...
3. **Use templates as guides:** Reference `docs/project/*.template.md` for structure only (do NOT copy placeholder text)
4. **Verify no placeholders remain:** Re-run grep/rg search after edits
5. **Update Last Updated date:** Set NEXT.md "Last Updated:" to today (YYYY-MM-DD)

### Exit criteria (Population Gate PASS)
- VISION.md: Purpose + North Star + User Promise + Non-Goals (all non-placeholder)
- EPICS.md: At least 1 epic with ID + status + description (all non-placeholder)
- NEXT.md: ACTIVE STORY ID + NEXT STEP + DoD + Scope + Done When (at least 3 items, NONE with placeholders) + Last Updated date (all non-placeholder)
...
- "If `docs/project/NEXT.md` is missing/unclear/outdated OR Control Deck Population Gate FAIL → STOP coding and enter Alignment Mode."
